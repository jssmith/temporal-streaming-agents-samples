...
import openai
from analytics_shared.constants import AUDIO_TOPIC
from analytics_shared.database import load_schema as _load_schema
from analytics_shared.sql_tool import execute_sql as _execute_sql
from analytics_shared.tts_chunking import MIN_FLUSH_LEN, SENTENCE_END
...
async def model_call(input: ModelCallInput) -> ModelCallResult:
    ...
    """
    batch_interval_s = float(os.environ.get("WORKFLOW_STREAM_BATCH_INTERVAL", "0.1"))
    stream = WorkflowStreamClient.from_within_activity(
        batch_interval=timedelta(seconds=batch_interval_s)
    )
    ...
    try:
        ...
        raise ApplicationError(f"Rate limited: {e}", type="RateLimitError") from e
        ...
        raise ApplicationError(f"Connection error: {e}", type="ConnectionError") from e
